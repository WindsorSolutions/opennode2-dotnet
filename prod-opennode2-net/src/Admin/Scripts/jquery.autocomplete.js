/*
 * Modified version to use getJSON instead of the ajax call.
 * Also supports space delimited autocomplete.
 * 
 * (c)2007 Windsor Solutions
 * 
 * 
 */
...
jQuery.Autocompleter = function(input, options) {
	...
	function trimWords(value) {
		...
		var words;
		if(options.multipleSeparator == " "){
			return value.split(options.multipleSeparator);
		}else{
			words = value.split( jQuery.trim( options.multipleSeparator ) );
		}
		var result = [];
	...
	function request(term, success, failure) {
		...
		// if an AJAX url has been supplied, try loading the data now
			...
			$.getJSON(
				options.url + "?q="+lastWord(term)+"&limit="+options.max,
				function(data) {
...
jQuery.Autocompleter.Select = function (options, input, select) {
	...
	function target(event) {
		...
		while(element != null && element.tagName != null && element.tagName != "LI")
