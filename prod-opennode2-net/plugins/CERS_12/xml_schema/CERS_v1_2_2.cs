...
namespace Windsor.Node2008.WNOSPlugin.CERS_12
{
    ...
    public partial class CERSDataType
    {
        ...
        [System.Xml.Serialization.XmlIgnore]
        [DbIgnore]
        public IList<string> AttachmentFileNames;

        public delegate void AttachedFileAction(string attachedFileName);

        public virtual IList<string> GetAttachmentFileNames()
        {
            List<string> fileNames = null;
            ForEachAttachedFile(delegate(string attachedFileName)
            {
                CollectionUtils.Add(attachedFileName, ref fileNames);
            });
            return fileNames;
        }
        public virtual int AddAttachmentsToZipFile(string zipFilePath, string attachmentParentFolderPath,
                                                   SpringBaseDao baseDao, ICompressionHelper compressionHelper)
        {
            int numAdded = 0;
            if (!string.IsNullOrEmpty(attachmentParentFolderPath))
            {
                ForEachAttachedFile(delegate(string attachedFileName)
                {
                    // TODO: blob handling not completed yet
                    string filePath = Path.Combine(attachmentParentFolderPath, attachedFileName);
                ...
                CollectionUtils.ForEach(FacilitySite,
                    delegate(FacilitySiteDataType facility)
                    {
                        if (facility.FacilitySiteAttachedFile != null)
                        {
                            ...
                            action(facility.FacilitySiteAttachedFile.AttachmentFileName);
                        }
                ...
                CollectionUtils.ForEach(Event,
                    delegate(EventDataType eventObj)
                    {
                        if (eventObj.EventAttachedFile != null)
                        {
                            ...
                            action(eventObj.EventAttachedFile.AttachmentFileName);
                        }
                    });
                CollectionUtils.ForEach(QualityFinding,
                    delegate(QualityFindingDataType qualityFinding)
                    {
                        if (qualityFinding.QualityFindingAttachedFile != null)
                        {
                            ...
                            action(qualityFinding.QualityFindingAttachedFile.AttachmentFileName);
                        }
                        CollectionUtils.ForEach(qualityFinding.QualityFindingOrganization,
                            delegate(QualityFindingOrganizationDataType qualityFindingOrganization)
                            {
                                if (qualityFindingOrganization.OrganizationAttachedFile != null)
                                {
                                    ...
                                    action(qualityFindingOrganization.OrganizationAttachedFile.AttachmentFileName);
                                }
                            });
                    });
            }
