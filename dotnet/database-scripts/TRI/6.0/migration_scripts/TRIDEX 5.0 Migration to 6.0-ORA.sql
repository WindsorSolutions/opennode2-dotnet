/*************************************************************************************************
** ObjectName: TRIDEX-ddl-migration-3.3-to-3.4-ORA.sql
**
** Author: Windsor Solutions, Inc.
**
** Company Name: Windsor Solutions, Inc.
**
** Description:  This script will update an existing Oracle TRIDEX 5.0 database to support the 
**               TRIDEX 6.0 application.  This script can be run multiple times without issue.  If
**               an alteration has already occurred it will be bypassed, otherwise the DDL 
...
** ------------------------------------------------------------------------------------------------
** 01/05/2016    Windsor     Added DDL to alter table EPARecon.  Added BIA Code and Tribe Name
**                           columns.
** 01/05/2016    Windsor     Added table TRI_COMMENT, primary key and foreign key.
** 01/05/2016    Windsor     Added DDL to alter table TRI_FAC added column PARENT_COMPANY_NAME_N_S.
** 01/05/2016    Windsor     Altered table TRI_ONSITE_RELEASE_Q added STREAM_REACH_CODE.
** 01/05/2016    Windsor     Altered table TRI_REPORT added 5 new columns.
** 01/05/2016    Windsor     Added function CleanAndTrim.
** 01/05/2016    Windsor     Adeed function udf_FormatPhone
** 01/05/2016    Windsor     Added script to drop unused views.
** 01/05/2016    Windsor     Added modification to view V_EXT_DIOXIN_OFF_WT.
** 01/05/2016    Windsor     Added view V_EXT_DIOXIN_ONSITE_RELEASE
** 01/11/2016    Windsor     Pulled TRI alterations out into a separate script file, this
**                           script now only contains TRIDEX specific database alterations.
** 01/13/2016    Windsor     Updated the udf_Format
** 03/21/2016    Windsor     Commented out script to drop unused views (incompatible with Oracle SQL Developer).
*************************************************************************************************/


/*  
 *  Add Column:  "EPARecon" (BIA Code)
 */
DECLARE
...
BEGIN
  ...
  SELECT 1
    ...
    FROM user_tab_cols
   WHERE user_tab_cols.table_name = 'EPARecon'
     AND user_tab_cols.column_name = 'BIA Code';
     
   dbms_output.put_line('The column BIA Code on table EPARecon has already been added, no changes required!');
...
EXCEPTION
  WHEN NO_DATA_FOUND THEN
     ...
     */
     v_sql_statement := 'ALTER TABLE "EPARecon" ADD "BIA Code" NVARCHAR2(255)';
     EXECUTE IMMEDIATE v_sql_statement;
     dbms_output.put_line(v_sql_statement);
     
     dbms_output.put_line('The column "BIA Code" was added to the database table "EPARecon".');
   
  WHEN OTHERS THEN
      dbms_output.put_line('An error occurred while adding the column "BIA Code" to the table "EPARecon", please investigate!');
      RAISE; 
END;
/

/*  
 *  Add Column:  "EPARecon" (Tribe Name)
 */
DECLARE
...
BEGIN
  ...
  SELECT 1
    ...
    FROM user_tab_cols
   WHERE user_tab_cols.table_name = 'EPARecon'
     AND user_tab_cols.column_name = 'Tribe Name';
     
   dbms_output.put_line('The column "Tribe Name" on table "EPARecon" has already been added, no changes required!');
...
EXCEPTION
  WHEN NO_DATA_FOUND THEN
     ...
     *  Add new column to the database table.
     */
     v_sql_statement := 'ALTER TABLE "EPARecon" ADD "Tribe Name" NVARCHAR2(255)';
     EXECUTE IMMEDIATE v_sql_statement;
     dbms_output.put_line(v_sql_statement);
     
     dbms_output.put_line('The column "Tribe Name" was added to the database table "EPARecon".');
   
  WHEN OTHERS THEN
      dbms_output.put_line('An error occurred while adding the column "Tribe Name" to the table "EPARecon", please investigate!');
      RAISE; 
END;
/
...
-- form-feed, and carriage-return (respectively), with a whitespace
-- (and then trims that off if it's still at the beginning or end, of course).
-- =============================================
CREATE OR REPLACE FUNCTION CleanAndTrim ( p_Str NVARCHAR2 )
RETURN NVARCHAR2 AS
...
BEGIN
       ...
       v_Result := LTRIM(RTRIM(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(
                   LTRIM(RTRIM(p_Str)), CHR(9), ' '), CHR(10), ' '), CHR(11), ' '), CHR(12), ' '), CHR(13), ' ')));

       RETURN v_Result;
END;
/

CREATE OR REPLACE FUNCTION udf_FormatPhone (p_PhoneNumber IN VARCHAR2)
RETURN VARCHAR2
AS

  v_Phone CHAR(2000);
  v_PhoneNumber VARCHAR2(4000);
  w_PhoneNumber VARCHAR2(4000);

BEGIN

  v_Phone := p_PhoneNumber;
  v_PhoneNumber := p_PhoneNumber;
  ...
  FOR i IN 1 .. LENGTH(v_PhoneNumber) LOOP 
  
    IF regexp_instr(substr(v_PhoneNumber,i,1),'[0-9].*') > 0 THEN
      w_PhoneNumber := w_PhoneNumber || substr(v_PhoneNumber,i,1);
    END IF;
  ...
  END LOOP;
    ...
    -- skip foreign phones
    IF (    SUBSTR(w_PhoneNumber,1,1) = '1'
         OR SUBSTR(w_PhoneNumber,1,1) = '+'
         OR SUBSTR(w_PhoneNumber,1,1) = '0')
       AND LENGTH(w_PhoneNumber) >= 11 THEN
      RETURN v_Phone;
    END IF;

    -- build US standard phone number
    v_Phone := w_PhoneNumber;
    w_PhoneNumber := '(' || SUBSTR(w_PhoneNumber,1,3) || ') ' || SUBSTR(w_PhoneNumber,4,3) || '-' || SUBSTR(w_PhoneNumber,7,4);

    DBMS_OUTPUT.PUT_LINE('v_Phone: ' || v_Phone);
    
    RETURN w_PhoneNumber;
...
BEGIN
  ...
  FOR v IN (SELECT view_name
              FROM user_views
             WHERE user_views.view_name IN ('V_EXT_MASTER_D'
                                           ,'V_EXT_DETAIL_D'
                                           ,'V_EXT_OFF_WT_D'
                                           ,'V_EXT_STR_D'
                                           ,'V_EXT_W_MGMT_D'
                                           ,'V_EXT_CONTACT_D'
                                           ,'V_EXT_DIOXIN_OFF_WT_D'
                                           ,'V_EXT_DIOXIN_D'
                                           ,'V_EXT_DIOXIN'
                                           ,'V_EXT_P2_FEE_D'
                                           )) LOOP
    v_sql_statement := 'DROP VIEW ' || v.view_name;
    EXECUTE IMMEDIATE v_sql_statement;                          
...
CREATE OR REPLACE VIEW V_EXT_CONTACT
AS
...
--       T.agencyname                                                AS AKA, 
       ...
       TO_CHAR(TO_DATE(r.cert_signed_date,'MM/DD/YYYY HH:MI:SS PM'),'MM/DD/YYYY') AS Received, 
...
FROM   tri_report R 
       ...
       LEFT OUTER JOIN v_fac_ident FI 
                    ON Upper(F.fac_id) = FI.trifid 
       LEFT OUTER JOIN tri_fac_naics N 
                    ON F.pk_guid = N.fk_guid 
                       AND N.naics_primary_ind = 'Primary' 
       LEFT OUTER JOIN app_lookup_cty_reg CTYREG 
                    ON CTYREG.county = F.county 
       LEFT OUTER JOIN app_fi_trifid T 
                    ON F.fac_id = T.trifid 
       LEFT OUTER JOIN app_fi_trifid_delete_chem D 
...
GROUP  BY F.pk_guid, 
          ...
          F.mail_fac_site, 
          TO_CHAR(TO_DATE(r.cert_signed_date,'MM/DD/YYYY HH:MI:SS PM'),'MM/DD/YYYY'), 
 ...
 *  View:  V_EXT_DETAIL
 */
CREATE OR REPLACE VIEW "V_EXT_DETAIL"
AS
SELECT
 FI.FS_ID AS FSID
, FI.EPA_ID AS "EPA ID"
...
, CASE TRI_CalculateRevisionIndicator(R.PK_GUID)
  WHEN 1 THEN 'Y'
  ELSE 'N'
 END AS Revision
, R.SUB_PARTIAL_FAC_ID AS "Partial Sub"
, SUBSTR(R.REPORT_TYPE_CODE,LENGTH(R.REPORT_TYPE_CODE)-1,1) AS Form
, F.FAC_SITE AS "Facility Name"
--, T.AgencyName AS AKA
, F.LOC_ADD_TXT AS "Address 1"
, F.SUPP_LOC_TXT AS "Address 2"
...
,
                          ...
                          (SELECT CAST(CAST(SUM(CASE WHEN ONSR.WASTE_Q_NA_IND = 'Y' 
                                                       THEN CAST(0 AS FLOAT) 
                                                       ELSE CAST(TRI_RangeCodeConversion(ONSR.WASTE_Q_ME, ONSR.WASTE_Q_RANGE_CODE) AS FLOAT) 
                                                     END) AS FLOAT) AS VARCHAR2(100))
                                                      AS Expr1
                             FROM TRI_ONSITE_RELEASE_Q ONSR
                            WHERE (FK_GUID = R.PK_GUID) 
                              AND (EI_MEDIUM_CODE = 'water')
                          ) AS "53 Water",
                          (SELECT     CASE WHEN ONSR.WASTE_Q_NA_IND = 'Y' THEN NULL ELSE TRI_RangeCodeConversion(ONSR.WASTE_Q_ME, 
                                                   ONSR.WASTE_Q_RANGE_CODE) END AS Expr1
                            FROM          TRI_ONSITE_RELEASE_Q ONSR
                          ...
                          (SELECT     SUM(CAST(CASE WHEN ONSR.WASTE_Q_NA_IND = 'Y' 
                                            THEN CAST(0 AS FLOAT) 
                                            ELSE CAST(TRI_RangeCodeConversion(ONSR.WASTE_Q_ME, ONSR.WASTE_Q_RANGE_CODE) AS FLOAT)
                                           END AS FLOAT)
                                               ) AS Expr1
                          ...
                          (SELECT     SUM(CAST(CASE WHEN OFSR.WASTE_Q_NA_IND = 'Y' THEN CAST(0 AS FLOAT) 
                                                   ELSE CAST(TRI_RangeCodeConversion(OFSR.WASTE_Q_ME, OFSR.WASTE_Q_RANGE_CODE) AS FLOAT) END AS FLOAT)) AS Expr1
...
, TO_CHAR(TO_DATE(r.cert_signed_date,'MM/DD/YYYY HH:MI:SS PM'),'MM/DD/YYYY') AS Signed
...
FROM         TRI_REPORT R INNER JOIN
                      ...
                      App_FI_TRIFID_Delete_chem D ON D.tri_report_id = r.pk_guid
WHERE NVL(R.REPORT_EPA_PROCESSING_STATUS_C, ' ') <> '5'
      ...
      (
       ...
       AND S.INSERTED_DATE = (SELECT MAX(S2.INSERTED_DATE)
                                ...
                                JOIN TRI_FAC F2 
                                  ON F2.FK_GUID = S2.PK_GUID 
                                JOIN TRI_REPORT R2 
                                  ON R2.FK_GUID = S2.PK_GUID 
                                JOIN TRI_CHEM C2 
                                  ON C2.FK_GUID = R2.PK_GUID
                               WHERE (F2.FAC_ID = F.FAC_ID) 
                                 AND (R2.SUB_REP_YEAR = R.SUB_REP_YEAR) 
                                 AND (C2.CAS_CLBER = C.CAS_CLBER) 
                                 AND (C2.CHEM_TXT = C.CHEM_TXT) 
                                 AND R2.SUB_PARTIAL_FAC_ID = 'N'
          )
 AND
   (
   TRI_CalculateRevisionIndicator(R.PK_GUID) = 1
   OR
    (
    TRI_CalculateRevisionIndicator(R.PK_GUID) = 0
    AND NOT EXISTS
     (
     SELECT 1 FROM TRI_REPORT R3
     ...
     AND
     TRI_CalculateRevisionIndicator(R3.PK_GUID) = 1
     )
    )
   )
 )
...
/*
 *  View:  V_EXT_DIOXIN_OFF_WT
 */
CREATE OR REPLACE VIEW "V_EXT_DIOXIN_OFF_WT"
AS
 SELECT
 TL.PK_GUID
, TL.FK_GUID
...
JOIN V_EXT_MASTER TD ON TL.FK_GUID = TD.PK_GUID_REPORT
JOIN TRI_POTW_WASTE_QUANTITY POTW ON POTW.FK_GUID = TD.PK_GUID_REPORT
JOIN TRI_TRANSFER_Q TLQ ON TLQ.FK_GUID = TL.PK_GUID
WHERE TD.REPORT_TYPE = 'TRI_FORM_R'
  AND TD.CAS_NUMBER = 'N150';
  

CREATE OR REPLACE VIEW "V_EXT_DIOXIN_ONSITE_RELEASE"
AS
...
,     V_FORM_S1_PG2.S1542NA AS "542 UG INJ Class II-V - TEQ_NA"
...
,     V_FORM_S1_PG2.S1551ANA AS "551A RCRA C Landfill - TEQ_NA"
...
,     V_FORM_S1_PG2.S1551BNA AS "551B Other Landfill - TEQ_NA"
...
/*
 *  View:  V_EXT_DIOXIN_SECTION_8
 */ 
CREATE OR REPLACE VIEW "V_EXT_DIOXIN_SECTION_8"
AS
SELECT V_FORM_S1_PG1.PK_GUID_SUB
,     V_FORM_S1_PG1.PK_GUID_REP
, T.AgencyID AS FSID
...
, R.SUB_PARTIAL_FAC_ID AS "Partial Sub"
--, RIGHT(R.REPORT_TYPE_CODE, 1) AS Form
, SUBSTR(R.REPORT_TYPE_CODE,LENGTH(R.REPORT_TYPE_CODE), 1) AS Form
, F.FAC_SITE AS "Facility Name"
-- , T.AgencyName AS AKA
, F.LOC_ADD_TXT AS "Address 1"
, F.SUPP_LOC_TXT AS "Address 2"
...
, C.CHEM_TXT AS Chemical
, C.CAS_CLBER AS "CAS #"
,     V_FORM_S1_PG4.S181ANA AS "81A Ons Disp UG INJ C1-TEQ_NA"
...
,     V_FORM_S1_PG4.S181CNA AS "81C Off Disp UG INJ C1-TEQ_NA"
...
CREATE OR REPLACE VIEW "V_EXT_EPA_MISSING_REPORTS" AS 
...
FROM TRI_SUB S
  ...
  (Replace(EPA."Chemical Number or Cat. Code",'-','')  = Replace(CAS_CLBER,'-','') 
    ...
    AND EPA."TRI Facility ID" = TRIFID 
          ...
          AND EPA."Reporting Form Type" = UPPER(COALESCE (SUBSTR(R.REPORT_TYPE_CODE,LENGTH(R.REPORT_TYPE_CODE), 1), ''))
  LEFT JOIN APP_FI_TRIFID_DELETE_CHEM D ON D.TRI_REPORT_ID = R.PK_GUID   
  LEFT JOIN App_FI_TRIFID FT ON F.FAC_ID = T.TRIFID       
...
-- =============================================
CREATE OR REPLACE VIEW "V_EXT_EPA_REPORTS_WITHDRAWN" AS 
SELECT DISTINCT r.sub_rep_year AS "Year"
     , f.fac_id AS "TRI Facility Id"
     , f.fac_site AS "Facility Name"
     , f.loc_add_txt AS "Street Address"
...
FROM TRI_SUB S
  ...
  (Replace(EPA."Chemical Number or Cat. Code",'-','')  = Replace(CAS_CLBER,'-','') 
    ...
    AND EPA."TRI Facility ID" = TRIFID 
          AND EPA."Year" = SUB_REP_YEAR
          AND EPA."Reporting Form Type" = UPPER(COALESCE (SUBSTR(R.REPORT_TYPE_CODE,LENGTH(R.REPORT_TYPE_CODE)-1, 1), ''))
  LEFT JOIN APP_FI_TRIFID_DELETE_CHEM D ON D.TRI_REPORT_ID = R.PK_GUID   
  LEFT JOIN App_FI_TRIFID FT ON F.FAC_ID = T.TRIFID       
WHERE EPA."TRI Facility ID" IS NULL AND LENGTH(CHEM_RPT_REV_CD_1) > 1 
  AND EXISTS (SELECT 1 FROM "EPARecon" WHERE "EPARecon"."Year" = R.SUB_REP_YEAR)
  AND D.TRIFID_DELETE_CHEM_ID IS NULL
  AND FT.Inactive_DT IS NULL;
    
  
CREATE OR REPLACE VIEW "V_EXT_EPA_RESPONSE" 
AS 
 ...
 */
CREATE OR REPLACE VIEW V_EXT_OFF_WT
AS
SELECT TL.PK_GUID
     , TL.FK_GUID
     , TD.FS_ID AS FSID
...
CREATE OR REPLACE VIEW "V_EXT_OPT_MISC_INF_TXT" 
AS 
WITH Facility_CTE
AS
(
SELECT R.PK_GUID
     , T.TRIFID
  , NVL(CASE WHEN NVL(T.AgencyName,'') = '' THEN NULL ELSE T.AgencyName END, F.FAC_SITE) "Facility Name"
     ...
     , CASE C.CAS_CLBER 
       ...
       END AS "DIOXIN_IND"
  , R.OPT_INF_CATG
  , REPLACE(CleanAndTrim(R.OPT_INF_TXT),CHR(34),'') OPT_INF_TXT
  , R.MISC_INF_CATG
  , REPLACE(CleanAndTrim(R.MISC_INF_TXT),CHR(34),'') MISC_INF_TXT
  , T.EPAID AS "EPA_ID"
  , F.FAC_SITE AS "FAC_NAME"
FROM TRI_SUB S
JOIN TRI_FAC F ON F.FK_GUID = S.PK_GUID
...
SELECT f."PK_GUID"
  ...
  END "Comment Section"
   ...
   ,REPLACE(CleanAndTrim(C.COMMENT_TEXT),CHR(34),'') "Comment Text"
...
/* =============================================
 ...
 =============================================*/
CREATE OR REPLACE VIEW V_EXT_P2_FEE
AS
...
CREATE OR REPLACE VIEW "V_EXT_SRC_RED_ACT"
AS
SELECT  s.PK_GUID PK_GUID_SUB
  ...
  ,  t.EPAID AS "EPA ID"
     ...
     , NVL(sra.SRC_RED_NA_IND,0) SRC_RED_ACT_NA_IND
     , sramc.SRC_RED_METH_CODE
     , NVL(sramc.SRC_RED_NA_IND,0) SRC_RED_MTHD_NA_IND
...
CREATE OR REPLACE VIEW "V_EXT_STR"
AS
SELECT 
       TD.fs_id                     AS FSID, 
       TD.epa_id                    AS "EPA ID", 
...
--       TD.agencyname                AS AKA, 
       ...
       CASE 
         ...
         ELSE Tri_rangecodeconversion(ONSR.waste_q_me, 
...
--       TD.agencyname, 
       ...
       TD.dioxin_ind 
FROM   v_ext_master TD 
       INNER JOIN tri_onsite_release_q ONSR 
...
WHERE  ( CASE 
           ...
           ELSE Tri_rangecodeconversion(ONSR.waste_q_me, 
                ONSR.waste_q_range_code) 
         END <> '0' ) 
       AND ( NVL(ONSR.stream, 'NA') <> 'NA' ) 
       AND ( TD.report_type = 'TRI_FORM_R' );  
       
       
CREATE OR REPLACE VIEW "V_EXT_TRI_SUMMARY" 
AS
SELECT DISTINCT tri_report.report_id                                 AS report_no 
                ...
                , CASE 
                    WHEN SUBSTR(tri_report.report_type_code, LENGTH(tri_report.report_type_code), 1) = 'R' THEN 'Y'
                    ELSE 'N' 
                  END                                                AS form_r10 
                , CASE 
                    WHEN SUBSTR(tri_report.report_type_code, LENGTH(tri_report.report_type_code), 1) = 'A' THEN 'Y'
                    ELSE 'N' 
                  END                                                AS form_a10 
                ...
                , CASE 
                    ...
                    ELSE 
                      CAST(CASE v_tri_potw_waste_quantity.waste_q_range_code 
                        WHEN 'A' THEN '5' 
                        WHEN 'B' THEN '250' 
                        WHEN 'C' THEN '750' 
                        ELSE '0'
                ...
                , CASE Tri_calculaterevisionindicator(tri_report.pk_guid) 
                    WHEN 1 THEN 'Y' 
                    ELSE 'N' 
                  END                                                AS rev_flag 
                , NULL                                               AS dispose_10 
...
CREATE OR REPLACE VIEW "V_EXT_UPDATE_FACILITY"
AS 
...
/* =============================================
 ...
 WHERE RIGHT(TD.REPORT_TYPE, 1) <> 'A'*/
CREATE OR REPLACE VIEW "V_EXT_W_MGMT"
AS
...
--       agencyname                             AS AKA, 
       ...
       (SELECT CASE 
        ...
        WHERE  ( fk_guid = TD.pk_guid_report ) 
               AND ( year_offset_me = '0' ))  AS 
       "OffDisp UGINJ Landfills CUR YR", 
       (SELECT CASE 
                 WHEN total_q_na_ind = 'Y' THEN NULL 
                 ELSE total_q 
               END AS Expr1 
       ...
       (SELECT CASE 
        ...
        WHERE  ( fk_guid = TD.pk_guid_report ) 
               AND ( year_offset_me = '2' ))  AS 
       "OffDisp UGINJ Landfill 2FWL YR", 
       (SELECT CASE 
                 WHEN total_q_na_ind = 'Y' THEN NULL 
                 ELSE total_q 
               END AS Expr1 
...
CREATE OR REPLACE VIEW "V_FORM_A_PG_1"
-- =================================================================================
...
SELECT DISTINCT
       ...
       udf_FormatPhone(R.TECH_CONT_PHONE_TXT) || CASE WHEN R.TECH_CONT_PHONE_EXT_TXT IS NOT NULL 
                                                   THEN NVL(' x' || R.TECH_CONT_PHONE_EXT_TXT,'') 
                                                 END AS A43TEL,
       PUB_CONT A44NAME,
       udf_FormatPhone(PUB_CONT_PHONE_TXT) || CASE WHEN R.PUB_CONT_PHONE_EXT_TXT IS NOT NULL
                                                THEN NVL(' x' || R.PUB_CONT_PHONE_EXT_TXT,'')
                                              END AS A44TEL,
...
CREATE OR REPLACE VIEW "V_TRI_PAGE1"
...
SELECT
       ...
       CASE CHEM_TRADE_SECRET_IND
           ...
           ELSE NULL
       END R22,
       NVL(CERTIFIER,'') || ' ' || NVL(CERTIFIER_TIT_TXT,'') AS R31,
       TO_CHAR(TO_DATE(r.cert_signed_date,'MM/DD/YYYY HH:MI:SS PM'),'MM/DD/YYYY') R33,
       FAC_ID R41TRI_ID,
       FAC_SITE R41NAME,
       NVL(F.LOC_ADD_TXT,'') || ' ' || NVL(F.SUPP_LOC_TXT,'') R41STREET,
       NVL(F.LOCALITY,'') || '/' || NVL(F.COUNTY,'') || '/' || NVL(F.STATE,'') || '/' || NVL(F.ADD_POSTAL_CODE,'') R41_CITYSTATE,
       F.MAIL_FAC_SITE R41MAIL_NAME,
       NVL(F.MAIL_ADD_TXT,'') || ' ' || NVL(F.SUPP_MAIL_ADD,'') R41MAIL_STREET,
       NVL(F.MAIL_ADD_CITY || '/','') || NVL(F.MAIL_ADD_STATE || '/','') || NVL(F.MAIL_ADD_POSTAL_CODE,'') R41MAILCITYSTATE,
       ...
       udf_FormatPhone(TECH_CONT_PHONE_TXT) || CASE WHEN TECH_CONT_PHONE_EXT_TXT IS NOT NULL
                                                 THEN NVL(' x' || TECH_CONT_PHONE_EXT_TXT,'')
                                               END AS R43TEL,
       PUB_CONT R44NAME,
       udf_FormatPhone(PUB_CONT_PHONE_TXT) || CASE WHEN PUB_CONT_PHONE_EXT_TXT IS NOT NULL
                                                THEN NVL(' x' || PUB_CONT_PHONE_EXT_TXT,'')
                                              END AS R44TEL,
...
CREATE OR REPLACE VIEW V_TRI_PG2_53
...
CREATE OR REPLACE VIEW V_TRI_PAGE2
...
CREATE OR REPLACE VIEW V_TRI_PAGE3
( PK_GUID_SUB
, PK_GUID_REP
, FAC_ID
, CHEM_TXT
...
CREATE OR REPLACE VIEW V_TRI_PAGE3_XFR AS
...
JOIN TRI_REPORT R ON R.FK_GUID = S.PK_GUID
JOIN TRI_CHEM C ON C.FK_GUID = R.PK_GUID
JOIN TRI_TRANSFER_LOC TL ON TL.FK_GUID = R.PK_GUID
LEFT JOIN
 (
 SELECT
  TQ.FK_GUID
...
CREATE OR REPLACE VIEW V_TRI_PAGE5_COMMENT
AS
SELECT
...
CREATE OR REPLACE VIEW V_TRI_PAGE5_CMT_ONE_LN
AS
SELECT "PK_GUID_SUB"
      ,"PK_GUID_REP"
      ,"COMMENT_SECTION"
      ...
      ,"COMMENT_TEXT"
      ,"COMMENT_P2_CLASS"
  FROM V_TRI_PAGE5_COMMENT V1

WHERE COMMENT_SEQ =
(
SELECT MIN(V2.COMMENT_SEQ)
FROM "V_TRI_PAGE5_COMMENT" V2
...
CREATE OR REPLACE VIEW V_TRI_PAGE5
AS
SELECT
S.PK_GUID AS PK_GUID_SUB,
R.PK_GUID AS PK_GUID_REP,
...
CASE WHEN NVL(R.PRODUCTION_RATIO_TYPE,'PRODUCTION') = 'PRODUCTION' THEN 'Y' ELSE 'N' END R89PRODYN
...
CREATE OR REPLACE VIEW V_TRI_PAGE5_SRA AS
SELECT R.FK_GUID PK_GUID_SUB
     , R.PK_GUID PK_GUID_REP
