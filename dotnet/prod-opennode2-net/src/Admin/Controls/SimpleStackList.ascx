...
<table id="formTable" width="100%" cellpadding="0" cellspacing="0">
    <asp:Repeater runat="server" ID="repeaterList">
        <ItemTemplate>
            <tr class="rowOdd" style='<%# Container.ItemIndex % 2 == 0 ? "background-color: #E3ECF3; color: #3366CC": "background-color: #DCDCDC; color: #3366CC"%>'>
                <td width="10px">
                    <img alt="" src='<%= Config.LinkItemImageUri %>' style="border: 0; vertical-align: middle; padding-right: 5px;" />
                </td>
                <td runat="server" id="valueNameItem" width="100%">
                    <a href='<%= Config.DetailItemLinkUri %>?id=<%# UriEscapeDataString(Eval("Key")) %>' class="listlabel" title="Edit"><strong>
                        <%# Server.HtmlEncode((string)Eval("Value.Name")) %></strong> </a>
                </td>
            ...
            <tr class="rowOdd" style='<%# Container.ItemIndex % 2 == 0 ? "background-color: #E3ECF3; color: #3366CC": "background-color: #DCDCDC; color: #3366CC"%>'>
                ...
                <td runat="server" id="valueDescriptionItem" width="100%" colspan="2" class="ctrl" align="left">
                    <a href='<%= Config.DetailItemLinkUri %>?id=<%# UriEscapeDataString(Eval("Key")) %>' class="listlabel" title="Edit">(<%# Eval("Value.Description")%>) </a>
                </td>
            </tr>
            <tr runat="server" id="customRow" class="rowOdd" style='<%# Container.ItemIndex % 2 == 0 ? "background-color: #E3ECF3; color: #3366CC": "background-color: #DCDCDC; color: #3366CC"%>' visible="false">
                <td>
                    &nbsp;
                </td>
                <td runat="server" id="customRowContent" width="100%" colspan="2" class="ctrl">
                </td>
            </tr>
        </ItemTemplate>
    </asp:Repeater>
